...
from app.seed_templates import bcse_home_market_indicators_graph, bcse_market_news_category_graph, bcse_news_graph
...
def test_bcse_home_indicators_graph_targets_only_currency_and_repo_widgets():
    graph = bcse_home_market_indicators_graph("source-3", "dataset-2", incremental=True)
    assert validate_dag(graph) == []
    nodes = {node["id"]: node for node in graph["nodes"]}
    ...
    assert fields["source_id"]["constant"] == "bcse-home-indicators"
    assert fields["selection_rule_id"]["constant"] == "bcse-home-currency-repo-v1"
    assert nodes["output"]["config"]["name"] == "market_indicators"


def test_bcse_home_widgets_extract_thirteen_typed_rows():
