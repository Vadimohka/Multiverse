...
NODE_CATALOG: list[dict[str, Any]] = [
    ...
    {"type": "crawl_links", "label": "Crawl Links / Новости", "category": "Fetch", "description": "Production fan-out: берёт URL из HTML/JSON, открывает каждую страницу с ограничением параллелизма, извлекает статью и возвращает записи. Подходит для полного архива БВФБ.", "fields": [
        field("listing_url", "URL списка / API", "template", placeholder="https://www.bcse.by/press_center/calendar"),
        field("listing_query", "Параметры списка", "json", default={}),
        field("lookback_days", "Окно последних дней (0 = параметры выше)", "number", default=0),
        field("input_path", "Путь к входному списку (без URL API)", default="records"),
        field("items_path", "Путь к массиву элементов", default="tabs.0.contents"),
        field("url_path", "Поле URL в элементе", default="url"),
        field("link_selector", "Selector ссылок (HTML-список)", "selector", default="a[href]"),
        field("url_pattern", "Regex ссылки и ID", default=r"/press-center/news/(n[^/?#]+)"),
        field("max_items", "Максимум материалов", "number", default=500),
        field("concurrency", "Параллельных запросов", "number", default=3),
        field("delay_ms", "Задержка после запроса, мс", "number", default=400),
        ...
        field("headers", "HTTP headers", "json", default={"Accept-Language": "ru-RU,ru;q=0.9,en;q=0.5"}),
        field("title_selector", "Selector заголовка", "selector", default="#title"),
        field("date_selector", "Selector даты", "selector", default=".dynamic-publicationdate"),
        field("body_selector", "Selector текста статьи", "selector", default="#pc_body"),
        ...
        field("language", "Язык записи", default="ru"), field("source_name", "Источник", default="БВФБ"),
