...
export function GuidedJsonEditor({fieldName,label,value,onChange}:Props){
  ...
  if(fieldName==='browserTraversalStates')return <label>{label}<BrowserTraversalStatesEditor value={value} onChange={onChange}/><AdvancedJson value={value} onChange={onChange}/></label>;
  if(fieldName==='browserTraversalFields'||fieldName==='extractFields')return <label>{label}<SelectorFieldsEditor value={value} onChange={onChange}/><AdvancedJson value={value} onChange={onChange}/></label>;
...
function BrowserTraversalStatesEditor({value,onChange}:{value:unknown;onChange:(value:unknown)=>void}){
  const states=Array.isArray(value)?value as Record<string,unknown>[]:[];
  function change(index:number,key:string,next:unknown){onChange(states.map((item,itemIndex)=>itemIndex===index?{...item,[key]:next}:item))}
  return <div className="editor-list"><div className="editor-list-head"><strong>Вкладки / фильтры</strong><button type="button" className="secondary small" onClick={()=>onChange([...states,{name:'state',actions:[]}])}>Добавить</button></div>{states.map((state,index)=><div className="editor-list" key={index}><div className="editor-row"><input aria-label={`state-${index}-name`} value={String(state.name||'')} placeholder="Название состояния" onChange={event=>change(index,'name',event.target.value)}/><button type="button" className="danger small" onClick={()=>onChange(states.filter((_,itemIndex)=>itemIndex!==index))}>×</button></div><BrowserActionsEditor value={state.actions||[]} onChange={next=>change(index,'actions',next)}/></div>)}</div>
}

function SelectorFieldsEditor({value,onChange}:{value:unknown;onChange:(value:unknown)=>void}){
  const fields=Array.isArray(value)?value as Record<string,unknown>[]:[];
  function change(index:number,key:string,next:unknown){onChange(fields.map((item,itemIndex)=>itemIndex===index?{...item,[key]:next}:item))}
  return <div className="editor-list"><div className="editor-list-head"><strong>Поля</strong><button type="button" className="secondary small" onClick={()=>onChange([...fields,{name:'field',selector:'',attribute:''}])}>Добавить</button></div>{fields.map((field,index)=><div className="editor-row" key={index}><input aria-label={`field-${index}-name`} value={String(field.name||'')} placeholder="Имя" onChange={event=>change(index,'name',event.target.value)}/><input aria-label={`field-${index}-selector`} value={String(field.selector||'')} placeholder="CSS selector" onChange={event=>change(index,'selector',event.target.value)}/><input aria-label={`field-${index}-attribute`} value={String(field.attribute||'')} placeholder="attribute" onChange={event=>change(index,'attribute',event.target.value)}/><button type="button" className="danger small" onClick={()=>onChange(fields.filter((_,itemIndex)=>itemIndex!==index))}>×</button></div>)}</div>
}
