...
async def build_selector_snapshot(url: str, timeout: float = 30) -> dict[str, Any]:
    try:
        ...
        async with async_playwright() as p:
            browser=await p.chromium.launch(headless=True); page=await browser.new_page(viewport={"width":1440,"height":900}); await page.goto(url,wait_until="networkidle",timeout=int(timeout*1000))
        ...
        return {"mode":"PLAYWRIGHT","url":url,"screenshot_base64":base64.b64encode(screenshot).decode(),"screenshot_size":screenshot_size,"elements":elements}
    except Exception as browser_error:
        async with httpx.AsyncClient(follow_redirects=True,timeout=timeout) as client: response=await client.get(url)
        ...
        return {"mode":"STATIC_HTML","url":url,"screenshot_base64":None,"elements":elements,"warning":f"Playwright недоступен, использован статический DOM: {browser_error}"}
